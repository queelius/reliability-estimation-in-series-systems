...
Introduction
============
Accurately estimating reliability of individual components in multi-component systems is challenging when only system-level failure data is observable. In a series system, only system failure times may be recorded along with limited information about the failed component in the form of candidate sets. Such masked data poses difficulties for assessing component reliability.
...
Deriving a likelihood model incorporating right-censoring and candidate sets to enable masked data to be used for parameter estimation.
Demonstrating through simulation studies that the maximum likelihood estimator performs well despite small samples and significant masking and right-censoring. Estimation of scale parameters is more robust than shape parameters.
Showing that bootstrapping provides reasonably well-calibrated confidence intervals for the maximum likelihood estimates, even with small sample sizes.
The remainder of the paper details the series system and likelihood models, maximum likelihood estimation methodology, bootstrap confidence interval estimation, and extensive simulation studies exploring estimator behavior under various sample sizes, masking levels, and model assumptions.
...
Together, these contributions provide a statistically rigorous framework for learning
about latent component properties from limited observational data on system reliability.
The proposed methods expand the capability to quantify component lifetimes in situations
where failure data is significantly masked.
...
product of the reliabilities of its individual components. This property is inherent to series
systems and will be used in the subsequent derivations. Next, we turn our attention to the pdf
of the system lifetime, described in the following theorem.
...
Theorem \@ref(thm:sys-pdf) shows the pdf of the system lifetime as a function of the pdfs and reliabilities of its components.
...
\end{align*}
By the property that $h_{T_i}(t_i;\v\theta) = f_{T_i}(t_i;\v\theta) / R_{T_i}(t_i;\v\theta)$,
we may do this substitution and simplify to obtain
$$
\Pr\{K_i = j|T_i = t_i\} = \frac{h_j(t_i;\v{\theta_j})}{\sum_{l=1}^m h_l(t_i;\v{\theta_l})}.
$$
:::
...
\end{equation}
which if certain assumptions are satisfied[^1] is equivalent to the integration of the reliability function over its support.

[^1]: $T_i$ is non-negative and continuous, $R_{T_i}(t;\v\theta)$ is a well-defined, continuous, and differential function for $t > 0$, and $\int_0^\infty R_{T_i}(t;\v\theta) dt$ converges.
...
system parameter $\v\theta$ from the masked data [@bain;casella2002statistical].
...
\end{equation}
for each component $\theta_j$ of the parameter $\v\theta$ [@bain]. When there's no closed-form solution,
...
Simulation Study: Full Model {#sim-study}
================================
...
well-designed and augment particularly small samples with
observations sampled in the following way:

1. Fit the *reduced* model (which assumes a well-designed
system with identical shape paramters) to the data.

2. Sample $M$ new observations, $\{T_i,\mathcal{C}_i\}_{i=n+1}^{n+M}$,
from the fitted model according to the DGP described in Section
\@ref(data-gen-proc). The number of new observations $M$ is a hyperparameter
that can be tuned, but intuitively we seek to sample enough new
observations to mitigate the masking and censoring effects.
For instance, this might suggest that we want the sample size to be
large enough so that each component, no matter how unlikely, is
represented in the sample as a masked component cause of failure.
...
of the model and the data. It is particularly useful for small
samples, where masking and censoring effects are more pronounced.

The DA can be tuned to work with the Bootstrap, where we can
resample $n$ observations from the unaugmented data, fit the
reduced model, sample $M$ new observations from the reduced model
fitted to the resampled data, and then fit the full model to the
augmented data. This would be a form of *augmented bootstrap*.
...
We have focused on the Weibull likelihood model, but we
have the general likelihood model for the series system.
We can allow the user to specify the parametric
for each component.


### Likelihood Model with Predictors {-}

In our current model, we only observe the failure times,
candidate sets, and right-censoring indicators. We can
extend the model to include additional predictors, e.g.,
let the hazard function be dependent on  predictors
$\v{w_1}, \ldots, \v{w_n}$, where $\v{w_i}$ is a vector
of predictors for the $i$th observation. Then, the
hazard function for the $j$th component is
$$
    h_j(t_i;\v{\beta_j} | \v{w_i}),
$$
for instance we may use the existing Weibull likelihood model,
but make the shape and scale parameters dependent on the
predictors $\v{w}_i$. We can use any parametric
likelihood model for the $j$th component. For instance,
we can use the Cox proportional hazards model,
$$
    h_j(t_i;\v{\beta_j}) = h_0(t_i) \exp(\v{\beta_j}^T \v{w_i}),
$$
where $h_{0}(t_i)$ is a baseline hazard function
shared by all components and $\v{\beta_j}$ is the parameter
vector for the $j$th component.

A more general model could parameterize each component
with its own hazard function, where we have the freedom
to choose any parametric form for the hazard function,
as long as it is a valid hazard function, by the observation
that
$$
    R_j(t_i;\v{\beta_j}|\v{w_i}) = \exp\{-\int_0^{t_i} h_j(t_i|\v{w_i};\v{\beta_j}\}.
$$
As long as $h_j(t|\v{w_i};\v{\beta_j})$ is non-negative and integrable,
then $R_j(t_i;\v{\beta_j}|\v{w_i})$ is a valid reliability function.

### Additional Likelihood Contributions {-}

This is related to the previous point, but we can extend the
likelihood model to include additional likelihood contributions.

We used masked component cause of failure and right-censoring
as the likelihood contributions in our model, but we can introduce
additional likelihood contributions, such as interval censoring,
left-censoring, other kinds of masking. This is also related
to relaxing the masking conditions, where we can introduce
additional likelihood contributions to account for the
relaxations.

### Prediction Intervals {-}

Often, what we are most interested in is *induction*, or prediction.
We want to predict the failure time of new series system, given
information in a sample,
$$
    T_{n + p}, \ldots, T_{n+1} | T_1 = t_1, \ldots, T_n = t_n.
$$

We might also want to predict a statistic of interest that is a
function of the new observations, e.g., the probability that
component $j$ is the component cause of failure,
$$
    \Pr\{K_{n + 1} = j | T_1 = t_1, \ldots, T_n = t_n\}.
$$
...
in the shape parameters, as described in Section \@ref(reduced-weibull). The reduced model offers
