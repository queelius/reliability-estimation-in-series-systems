...
probability that the $i$\textsuperscript{th} system has a lifetime longer than a duration $t$,
\begin{equation}
R_{T_i}(t) = \Pr\{T_i > t\}\\
\end{equation}
...
component and $R_l(t;\v{\theta_l})$ is the reliability function of the $l$\textsuperscript{th}.
component.
:::
...
We see that Theorems \@ref(thm:f-k-and-t) and \@ref(thm:prob-k) are closely related:
$$
  \Pr\{K_i = j\} = E_{\v\theta} \bigl[ \Pr\{K_i = j|T_i\} \bigr].
$$
...
We generally encounter two types of system failure lifetime masking:

- A system failure is observed to occur within a particular interval of time.

- A system failure is not observed, but we know that the system survived at least
...
In the case of masking the component cause of failure, we may not know the precise
component cause of failure, but may have some indication. In this paper, we sometimes
do not know which component failed, but can isolate the failed component to some subset
of the components, known as a candidate set, which is a subset of $\{1,\ldots,m\}$.
For example, if the candidate set is $\{1,2\}$, then we know that either the first
or second component failed, but not which one. In this case, we say that the
component cause of failure is *masked*. [^200]

[^200]: If the candidate set only contains a single
component, then we know the component cause of failure exactly.

In this paper, each observation is either right censored or the lifetime of the
system is observed precisely (no censoring) but with the component cause of failure
potentially masked.
We consider a sample of $n$ i.i.d. series systems, each of
which is put into operation and observed until either it fails
or is right-censored after some time duration $\tau$. 
...
If a system failure event is observed, then we also observe a candidate set
...
variations or unknown factors, we model it with a distribution.
...
\item{$\delta_i$} is the right-censoring indicator of the $i$\textsuperscript{th} system.
...
L_i(\v\theta) = \prod_{l=1}^m R_l(t_i;\v{\theta_l}) \sum_{j \in c_i} h_j(t_i;\v{\theta_j}).
\end{equation}
...
- As $q increases ($\tau$ increases), $R_{T_i}(\tau;\v\theta)$ decreases, and so the effect right-censoring has on the MLE decreases. This is what we see in
