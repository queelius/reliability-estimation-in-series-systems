...
\end{split}
\end{equation}
which we find to be a more convenient form than Equation \@ref(eq:sys_pdf).
...
Equation \@ref(eq:joint_pdf_t_k_c),
$$
...
contribution is given by Equation \@ref(eq:likelihood_contribution_masked).
...
Substituting $L_i(\v\theta)$ from Equation \@ref(eq:like), we consider these two cases of $\delta_i$
...
Equations \@ref(eq:sys_weibull_reliability_function) and
\@ref(eq:sys_weibull_failure_rate_function) completes the proof.
:::
...
Equation \@ref(eq:prob_k) is a useful measure of component reliability compared to
...
::: {.theorem #thm:weibull-likelihood-contribution}
...
\end{cases}
$$
By Equation \@ref(eq:sys_weibull_reliability_function), the system reliability
function $R_{T_i}$ is given by
$$
R_{T_i}(t_i;\v\theta) = \exp\biggl\{-\sum_{j=1}^{m}\biggl(\frac{t_i}{\lambda_j}\biggr)^{k_j}\biggr\}.
$$
and by Equation \@ref(eq:weibull_haz), the Weibull component hazard function $h_j$ is
given by
$$
...
We find an MLE by solving \@ref(eq:mle_eq), i.e., a point
...
::: {.theorem #weibull_score}
The gradient of the log-likelihood contribution of the $i$-th system is
given by
\begin{equation}
...
\@ref(eq:weibull_likelihood_contribution). It is a tedious calculation so the proof
...
Theorem \ref{thm:weibull_score}. Let us analyze the implications these
...
The IQR (vertical blue bars) decreaes in length as the right-censoring quantile
increases. This suggests that the bootstrapped CIs are getting more consistent
and focused around a narrower range as the right-censoring quantile
increaes, while maintaining a good coverage probability. As right-censoring
events become less likely, the bootstrapped CIs are more likely to be
closer to each other and the true value of the parameters.

For small right-censoring quantiles (small right-censoring times), they are
quite large, but to maintain well-calibrated CIs, this was necessary. The
estimator is quite sensitive to the data, and so the bootstrapped CIs are quite
wide to account for this sensitivity when the sample contains insufficient
...
This has the following implications this has on the MLE:

- **Ambiguity**: A higher $p$ makes larger candidate
sets more probable, making it less clear for the MLE which parameters should be
adjusted to make the data more likely. This is particularly problematic for
components that are not the cause of failure, since the MLE will adjust their
parameters to make them more likely to be the cause of failure, which is
not necessarily correct, except in the case of a well-designed series system.
...
series system, this may be an ideal adjustment, but if the system deviates from
...
right-censoring and masking. The background material also suggests exploring
the affect of deviations in system design in the presence of masking.
...
reinforcing the role of sample size in achieving robust estimates. These
findings pave the way for our next scenario, which will explore how deviations
in system design can further impact these observations.
