...
In our simulation study, we parameterize the right-censoring time $\tau$ by the
...
- Condition \@ref(cond:equal-prob-failure-cause) may be the least intuitive of
  ...
  compoent $1$ or component $2$ being the cause of failure, which satisfies
...
We also see that the coverage probabilities for $k_1$ and $\lambda_1$ are
...
likely to fail before the right-censoring time $\tau$. This means that the
likelihood contribution of component 1 is less informative than the other
components. Conversely, we see that $k_3$ and $\lambda_3$ generally have
...
right-censoring), and we vary the masking probability $p$ from $0.1$
...
I also thank Dr. Ed Sewell for his willingness to contribute to my committee.
