...
(i.i.d.).
The lifetime of the $i^{th}$ system is denoted by the random
variable $T_i$ and the lifetime of its $j^{th}$ component is
...
the $i^{th}$ system is given by the component with the shortest
lifetime,
$$
...
probability that the $i^{th}$ system has a lifetime greater than 
...
The lifetime of the $j^{th}$ component is assumed to follow a parametric distribution indexed
by a parameter vector $\v{\theta_j}$. The parameter vector of the overall system is defined as
$$
    \v\theta = (\v{\theta_1},\ldots,\v{\theta_m}),
$$
where $\v{\theta_j}$ is the parameter vector of the $j^{th}$ component.
...
the pdf of the $j^{th}$ component is denoted by $f_j(t;\v{\theta_j})$. Two continuous
...
$$
(\#eq:sys-reliability-fn)
R_{T_i}(t;\v\theta) = \prod_{j=1}^m R_j(t;\v{\theta_j}),
$$
where $R_j(t;\v{\theta_j})$ is the reliability function of the $j^{th}$ component.
:::
...
$$
where $f_j(t;\v{\theta_j})$ is the pdf of the $j^{th}$ component
and $R_k(t;\v{\theta_k})$ is the reliability function of the $k^{th}$ component.
:::
...
$$
where $h_j(t;\v{\theta_j})$ is the hazard function of the $j^{th}$ component.
:::

::: {.proof}
By Equation \@ref(eq:failure-rate), the $i^{th}$ series system
...
We denote the component cause of failure of the $i^{th}$ series
...
$$
(\#eq:f-k-and-t)
  f_{K_i,T_i}(j,t;\v\theta) = h_j(t;\v{\theta_j}) \prod_{l=1}^m R_l(t;\v\theta_l),
$$
where $h_l(t;\v{\theta_l})$ and $R_l(t;\v{\theta_l})$ are respectively the hazard
and reliability functions of the $l^{th}$ component.
:::
...
$$
where $f_j(t_j;\v{\theta_j})$ is the pdf of the $j^{th}$ component.
...
The probability that the $j^{th}$ component is the cause of failure
...
$i^{th}$ system and $T_i$ is the random variable denoting the
lifetime of the $i^{th}$ system.
:::

::: {.proof}
The probability the $j^{th}$ component is the cause of failure is given by
...
for the probability that the $j^{th}$ component is the cause of
failure. This is given by the following theorem.

::: {.theorem #prob-k-given-t}
The probability that the $j^{th}$
...
e.g., $R_{T_i}(t';\v\theta)$ denotes the probability that the $i^{th}$
...
of the system. We denote the MTTF of the $j^{th}$ component by
$\text{MTTF}_j$ and, according to Theorem \@ref(thm:prob-k), the probability
that the $j^{th}$ component is the cause of failure is given by
...
Here, $\delta_i = 1$ indicates the $i^{th}$ system's failure was
...
candidate set for the $i^{th}$ system by $\mathcal{C}_i$, which
...
- $S_i$ is the right-censored system lifetime of the $i^{th}$ system.
- $\delta_i$ is the event indicator for the $i^{th}$ system.
- $\mathcal{C}_i$ is the set of candidate component causes of failure for the
$i^{\text{th}}$ system.


```{r depmodel, fig.center=TRUE, echo=FALSE, fig.cap = "Dependence model for the data generating process for the masked data."}
knitr::include_graphics(if (knitr::is_latex_output()) "./image/dep_model_standalone.pdf" else "./image/dep_model_standalone.png")
```

In Figure \@ref(fig:depmodel), we depict a dependency graph of the data generating
process for $D_i = (S_i, \delta_i, \mathcal{C}_i)$. The elements in green are
observed in the sample, while the elements in red are unobserved (latent). We
see that $\mathcal{C}_i$ is related to both the unobserved component lifetimes
$T_{i1},\ldots,T_{im}$ and other unknown or unobserved covariates, like ambient
temperature or the particular diagnostician who generated the candidate set.
These two complications for $\mathcal{C}_i$ are why we seek a way to construct a
reduced likelihood function in later sections that does not need to model the
distribution of $\mathcal{C}_i$.

An example of masked data $D$ with a right-censoring time $\tau = 5$ can be seen
in Table \@ref(tab:masked-data) for a series system with $3$ components.

Table: (\#tab:masked-data) Right-censored lifetime data with masked component cause of failure.

System | Censored Lifetime ($S_i$) | Event ($\delta_i$) | Candidates ($\mathcal{C}_i$) |
:----: | :-----------------------: | :----------------: | ---------------------------: |
1      | $1.1$                     | 1                  | $\{1,2\}$                    |
2      | $1.3$                     | 1                  | $\{2\}$                      |
4      | $2.6$                     | 1                  | $\{2,3\}$                    | 
...
indicator, and $c_i$ is the observed candidate set of the $i^{th}$ system.
...
$$
is the likelihood contribution of the $i^{th}$ system.
...
$$
where $\delta_i = 0$ indicates the $i^{th}$ system is
right-censored at time $s_i$ and $\delta_i = 1$ indicates the $i^{th}$ system
...
data, $\mathcal{C}_i$, is correlated with the $i^{th}$ system.
...
$i^{th}$ system is given by assuming the following condition.
\begin{condition}
\end{condition}

**Condition 1.** The candidate set $\mathcal{C}_i$ contains the index of the failed component, i.e.,
$$
\Pr{}_{\!\v\theta}\{K_i \in \mathcal{C}_i\} = 1,
$$
where $K_i$ is the random variable for the failed component index of the
$i^{th}$ system. (cond:c-contains-k)
...
and reliability functions of the $j^{th}$ component.
...
According to [@Fran-1991], many industrial problems feature masking due to time
...
\ref{cond:equal-prob-failure-cause}, and \ref{cond:masked-indept-theta}, 
if we observe an exact system failure time for the $i^{th}$
...
evaluated at $s_i$,
$$
L_i(\v\theta) = R_{T_i}(s_i;\v\theta) = \prod_{l=1}^m R_l(s_i;\v{\theta_l}).
$$
:::

When we combine the two likelihood contributions, we obtain the likelihood
contribution for the $i^{th}$ system shown in Theorem
...
$$
where $\ell_i(\v\theta)$ is the log-likelihood contribution for the $i^{th}$ observation:
$$
...
The $j^{th}$ component of the $i^{th}$ system has a
...
The $j^{th}$ component has a reliability function, pdf, and hazard function
...
\begin{align}
    ...
    h_j(t;\lambda_j,k_j) (\#eq:weibull-haz)
        &= \frac{k_j}{\lambda_j}\biggl(\frac{t}{\lambda_j}\biggr)^{k_j-1}.
\end{align}
...
parameter vector of the $j^{th}$ component.
:::
...
components, the MTTF of the $j^{th}$ component is given by
$$
(\#eq:mttf-weibull)
...
If $k_j > 1$, the lifetime distribution of the $j^{th}$ component
...
$i^{th}$ system is given by the following theorem.

::: {.theorem #weibull-likelihood-contribution}
Let $\delta_i$ be an indicator variable that is 1 if the
$i^{th}$ system fails and 0 (right-censored) otherwise.
Then the likelihood contribution of the $i^{th}$ system is given by
$$
(\#eq:weibull-likelihood-contribution)
L_i(\v\theta) \propto
\begin{cases}
...
Equation \@ref(eq:weibull-haz), the hazard function of the $j^{th}$
...
$j^{th}$ component. Plugging these into the likelihood contribution
...
$$
(\#eq:weibull-log-likelihood-contribution)
\ell_i(\v\theta) =
...
given by
$$
(\#eq:sys-weibull-scale)
\lambda = \biggl(\sum_{j=1}^{m} \lambda_j^{-k}\biggr)^{-1/k},
$$
where $\lambda_j$ is the scale parameter of the $j^{th}$ component.
:::
...
$j^{th}$ component being the cause of failure given the system
...
lifetimes. As described in Section \@ref(weibull), the $j^{th}$
component of the $i^{th}$ system has a lifetime distribution
given by
$$
...
method, which we call the *Bernoulli masking model*. In this model, we
...
- If the $j^{th}$ component fails, it is deterministically
...
probability. If $X_j = 1$, the $j^{th}$ component is placed in
...
[^88]: See 'Table 2: Example Data for a Series System' in [@Huairu-2013] for details on the masked data.
...
\begin{align*}
\frac{\partial R_{T_i}(\tau_q)}{\partial \lambda_j} &= R_{T_i}(\tau_q;\v\theta) \left(\frac{\tau}{\lambda_j}\right)^{k_j} \frac{k_j}{\lambda_j},\\
\frac{\partial R_{T_i}(\tau_q)}{\partial k_j}       &= R_{T_i}(\tau_q;\v\theta) \left(\frac{\tau}{\lambda_j}\right)^{k_j} \left(\log \lambda_j - \log \tau_q\right),
\end{align*}
...
quantile $q$ increases ($\tau_q$ increases), $R_{T_i}(\tau_q;\v\theta)$ decreases,
...
to the shape parameter of the $j^{th}$ component, $k_j$, is
...
- We vary the Bernoulli masking probability $p$ from $10\%$ to $70\%$.[^99]
...
[^99]: The probability that the component cause of failure is masked is the
...
large. At $p = 0.4$, the coverage probability is around $90%$, but continues to
...
intervals for the parameter $\v\theta$, which generated correctly specified
...
score_wei_series_md_c1_c2_c3 <- function(df, theta) {
  ...
  scr[seq(1, length(theta), 2)] <- shapes.scr
  scr[seq(2, length(theta), 2)] <- scales.scr
  return(scr)
}
```
...
in which the $k^{th}$ iteration is given by
$$
...
[@towell2023reliability]. For clarity and brevity, certain functionalities and
