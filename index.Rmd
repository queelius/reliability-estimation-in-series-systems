...
confidence intervals (CI) constructed for the MLE. We found that the CIs are
reasonably well-calibrated even for small sample sizes in the presence of
significant masking and censoring, but that the CIs for the shape parameters
were less well-calibrated, indicating that the shape parameters are more
difficult to estimate than the scale parameters.
...
- **Convergence Rate**: In Section \@ref(identifiability), we discussed
  ...
  converged to a local maximum. We take low convergence rates, say a convergence
  rate less than $95\%$, to be evidence that the MLE is not a robust or reliable
  estimator in these cases, due to the lack of information in the data to
  identify the parameters of the likelihood model. Under these conditions, the
  MLE should be taken with a grain of salt.
...
For the sake of clarity and brevity within this appendix, certain
functionalities and safeguards from the original code have been omitted.
However, the full, unedited code, as well as the methods to reproduce this
paper, are available in the GitHub repository.
...
experiment_name <- "data" # filename prefix for output files

#### Simulation code below here ####
file.meta <- paste0(experiment_name, ".txt")
file.csv <- paste0(experiment_name, ".csv")
      ...
      for (iter in 1L:R) {
        tryCatch({
          ...
          row <- data.frame(
            ...
            convergence = sol$convergence, iter = sol$iterations,
...
relating the sampling distribution of the MLE to some simulation parameter,
like sample size ($n$) or masking probability ($p$). In the code below, we
set the simulation parameter to be the masking probability $p$.
...
def plot_x_vs_mle(raw_data, x_col, par):
  ...
  for i, x in enumerate(x_vals):
    ...
    low_med, up_med = np.percentile(data[par_low], 50), np.percentile(data[par_up], 50)
    ...
    plt.vlines(i, low_med, up_med, color='blue', label='Median 95% CI' if i == 0 else "")
