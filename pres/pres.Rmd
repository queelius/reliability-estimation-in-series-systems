...
- Aim for precise and accurate reliability estimates for individual components
  using maximum likelihood estimation (MLE) and confidence intervals (BCa).
...
- **Right-Censored**: Experiment ends before failure (Event Indicator: $\delta_i = 0$).
  - Contribution is system reliability: $L_i(\v\theta) = R_{T_i}(\tau;\v\theta)$.
    - Since we only know that it lasted longer than the right-censoring time $\tau$.
...
\item Our model deals two kinds of \textbf{contributions}, right-censoring and masked failures.
\item \textbf{Right-censoring}, for instance, occurs when the experiment ends before the system fails. Its contribution is just the system reliability,
since we only know that it lasted longer than the right-censoring time.
...
Weibull has well known *reliability* and *hazard* functions.
...
\item If it's \textbf{equal} to one, it has random failures.
\end{itemize}
}

## Well-Designed Series System

**Simulation study** centered on a series system with these Weibull components:
...
\item Since the probabilities are \textbf{comparable}, no weak links, it's reasonably \textbf{well-designed}.
\item Component 1 is the \textbf{most} reliable and component 3 is the \textbf{least} reliable.
