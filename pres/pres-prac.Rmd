...
$95\%$ of the time.
\item To obtain good coverage, we bootstrap the confidence intervals using the BCa method.
\end{itemize}
}
...
\begin{itemize}
\item We've \textbf{derived a likelihood model} for series systems that accounts for
\textbf{right-censoring} and \textbf{masked component failure}.
...
\item The simulation is based on components with \textbf{Weibull} lifetimes.
...
\note{
\begin{itemize}
\item Many complex systems have \textbf{Critical Components} that
are essential to their operation.
\item If any of these components fail, the entire system
fails. We call these \textbf{Series System}.
\item Think of a \textbf{Car}- if the engine or brakes fail, it can't
be operated.
\item Its \textbf{Lifetime} is the lifetime of its \textbf{shortest-lived
component}.
\item For reference, we show the some notation we'll use
throughout the talk. $T_i$ is the system's lifetime and $T_{i j}$ is the
$j$\textsuperscript{th} component's lifetime.
\end{itemize}
}
...
\item \textbf{Right-censoring} time is the minimum of the system lifetime and the
right-censoring time.
\item \textbf{Event} indicator is 1 if the system fails before the right-censoring time, 0 otherwise.
\item The candidate sets are related to the component lifetimes and many other factors.
\item \textbf{Difficult} to model. Seek a simple model that is valid under certain
assumptions, which we discuss next.
\end{itemize}
}
...
candidate set is constant across different components within it.

**Masking Probabilities Independent of Parameters**: The masking probabilities
when conditioned on the system lifetime and the failed component aren't functions of the system parameter.

- Reasonable conditions in many industrial settings.
...
\item \textbf{Tractable}: To make problem more tractable, we introduce certain conditions.
\item \textbf{Condition 1}: The candidate set always includes the failed component.
\item \textbf{Condition 2}: The probability of the candidate set is constant across different components within it.
\item \textbf{Condition 3}: The masking probabilities when conditioned on the system lifetime and the failed component aren't functions of the system parameter.
\item \textbf{Reasonable}: These conditions are often reasonable.
\end{itemize}
}
...
\note{
\begin{itemize}
\item \textbf{Derive}: Here, we derive the likelihood contribution for masked failures.
\item \textbf{Joint}: To start, we use the joint distribution of the system lifetime, the failed component, and the candidate set.
\item \textbf{Marginalize}: We marginalize over the failed component, since we don't know which component failed.
\item Apply Condition 1 to get a sum over the candidate set instead.
\item Apply Condition 2 to move the probability outside the sum.
\item Apply condition 3 to remove the probability's dependence on $\v\theta$.
\item The result: the likelihood contribution is proportional to the product of the system reliability and the sum of the component hazards in the masking set.
\end{itemize}
}
...
- Fixed the masking probability $p$ to $21.5\%$
- Fixed the sample size $n$ to $90$.
...
- **Coverage**: CIs converge to $95\%$. Scale parameters better specified.
...
- Fixed the right-censoring quantile $q$ to $0.825$.
- Fixed the sample size $n$ to $90$.
...
- Scale parameter CIs are correctly specified up to $p = 0.7$.
- Shape parameter CIs drop below $90\%$ if $p > 0.4$.

**Convergence Rate**: Drops below $95\%$ if $p > 0.4$, consistent with CP behavior.


## Conclusion

- Masking influences MLE precision, coverage probability, and introduces bias.
- The bias is positive and increases with $p$, essentially adding a
  censoring-like effect to the components.
...
- Fixed the masking probability $p$ to $0.215$.
- Fixed the right-censoring quantile $q$ to $0.825$.
