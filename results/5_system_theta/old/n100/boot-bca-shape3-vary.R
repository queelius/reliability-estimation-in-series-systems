...
theta.star <- c(shape1 = 1.2576, scale1 = 994.3661,
                ...
                shape3 = 1.1308, scale3 = 500, #840.1141,
                shape4 = 1.1802, scale4 = 940.1342,
                shape5 = 1.2034, scale5 = 923.1631)
shapes3 <- c(1.2034, .75, .635, .55, .5, .456, 2.5, 5, 10)

# MTTF(3) = 500*gamma(1 + 1/c(.456, .5, .55, .635, .75, 1.1308))
#         = 1203.5280 1000.0000 851.2145 701.5169 595.3197 478.2469
    ...
    for (n in N) {
        for (p in P) {
            for (q in Q) {
                ...
                cat("[starting] scenario(shape3: ", shape3, ", n: ", n, ", p: ", p, ", q: ", q, ")\n")
                cat("tau: ", tau, "\n")
                cat("theta: ", theta, "\n") 
                ...
                write.table(df, file = "boot-bca-shape3-vary.csv", sep = ",", row.names = FALSE,
                    col.names = !file.exists("boot-bca-shape3-vary.csv"), append = TRUE)
            }
        }
    }
}
