...
tau <- wei.series.md.c1.c2.c3::qwei_series(
    ...
    optim(
        ...
        control = list(maxit=10000L, lmm = 1000, fnscale = -1, parscale = parscale, factr=1e-100,pgtol = 1e-100),
        hessian = TRUE))


sol.dist <- matrix(NA, nrow = R, ncol = length(theta))
...
#x <- algebraic.dist::empirical_dist(neld.dist)
#y <- algebraic.dist::empirical_dist(lbfgsb.dist)
