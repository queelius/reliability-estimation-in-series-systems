...
def plot_mles_and_cis(ax, data_p, true_params, param, param_label):
    ...
    data_p_no_outliers = data_p #data_p.groupby('p').apply(remove_outliers_iqr, param).reset_index(drop=True)
...
data = pd.read_csv('/home/spinoza/github/private/proj/results/ci_vs_p/data-boot-tau-fixed-bca-p-vs-ci-n90-cat.csv')
...
# Show the plot
#plt.show()
plt.savefig('plot-p-vs-stats-scale-n90.pdf', dpi=100)
plt.savefig('plot-p-vs-stats-scale-n90.png', dpi=300)
