...
for j, color, ls, mk in zip(range(1, 6), scale_cmap(np.linspace(0.4, 1, 5)), line_styles, markers):
    if (j == 3):
        # Special case for scale 3. let's highlight it in a different color that is VERY EASY
        # to distinguish from the other colors, use a BRIGHT PINK color
...
plt.xlabel('Probability of 3rd Component Failure ($\Pr\{K_i = 3\}$) By Varying Scale')
