...
relevant_columns = ['q'] + [f'shape.{j}' for j in range(1, 6)] + [f'scale.{j}' for j in range(1, 6)] \
...
# Grouping by 'q' and computing the average coverage probability
coverage_columns = [f'shape_coverage.{j}' for j in range(1, 6)] + [f'scale_coverage.{j}' for j in range(1, 6)]
coverage_probabilities = relevant_data.groupby('q')[coverage_columns].mean().reset_index()

# Renaming columns for clarity
coverage_probabilities.columns = ['q'] + [f'shape_coverage_prob_{j}' for j in range(1, 6)] + [f'scale_coverage_prob_{j}' for j in range(1, 6)]


# Extracting the columns containing the coverage probabilities for shape and scale parameters
shape_coverage_columns = [f'shape_coverage_prob_{j}' for j in range(1, 6)]
scale_coverage_columns = [f'scale_coverage_prob_{j}' for j in range(1, 6)]
...
for j in range(1, 6):
    plt.plot(coverage_probabilities['q'], coverage_probabilities[f'shape_coverage_prob_{j}'], label=f'$k_{j}$')
plt.plot(coverage_probabilities['q'], coverage_probabilities['mean_shape_coverage_prob'], color='magenta', linewidth=3, label='Mean Shape')
plt.axhline(y=0.95, color='r', linestyle='--', label='Nominal 95% Level')
plt.xlabel('Right-Censoring Quantile ($q$)')
...
plt.figure(figsize=[4,4])
for j in range(1, 6):
    plt.plot(coverage_probabilities['q'], coverage_probabilities[f'scale_coverage_prob_{j}'], label=f'$\lambda_{j}$')
plt.plot(coverage_probabilities['q'], coverage_probabilities['mean_scale_coverage_prob'], color='magenta', linewidth=3, label='Mean Scale')
plt.axhline(y=0.95, color='r', linestyle='--', label='Nominal 95% Level')
plt.xlabel('Right-Censoring Quantile ($q$)')
plt.ylabel('Coverage Probability (CP)')
