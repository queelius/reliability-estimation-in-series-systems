...
# Extract columns
relevant_columns = ['n'] + [f'shape.{i}' for i in range(1, 6)] + [f'scale.{i}' for i in range(1, 6)] + \
...
for j, color, ls, mk in zip(range(1, 6), shape_cmap(np.linspace(0.4, 1, 5)), line_styles, markers):
    plt.plot(coverage_probabilities['n'], coverage_probabilities[f'shape_coverage.{j}'], label=f'Shape $k_{j}$', color=color, linestyle=ls, marker=mk)

for j, color, ls, mk in zip(range(1, 6), scale_cmap(np.linspace(0.4, 1, 5)), line_styles, markers):
    plt.plot(coverage_probabilities['n'], coverage_probabilities[f'scale_coverage.{j}'], label=f'Scale $\lambda_{j}$', color=color, linestyle=ls, marker=mk)

plt.plot(coverage_probabilities['n'], coverage_probabilities['mean_shape_coverage_prob'], color='darkblue', linewidth=4, linestyle='-', label='Mean Shape')
plt.plot(coverage_probabilities['n'], coverage_probabilities['mean_scale_coverage_prob'], color='darkred', linewidth=4, linestyle='-', label='Mean Scale')
...
#plt.legend(loc='best')
plt.tight_layout()
#plt.savefig('combined-cp-improved.pdf', bbox_inches='tight')
plt.savefig('combined-cp-improved.pdf')
