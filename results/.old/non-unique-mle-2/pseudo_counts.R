...
l.post <- Vectorize(function(shape1) {
    theta <- sol.post$par
    theta[1] <- shape1
...
yrange <- range(c(l(shape1), l.post(shape1)))
...
plt.tweaked <- ggplot(data.frame(x=shape1, y=l.post(shape1)), aes(x=x, y=y)) +
    geom_line() +
    geom_vline(xintercept = shapes[1], linetype="dashed", color = "blue", size=1) + # Add true k1
    geom_vline(xintercept = sol.post$par[1], linetype="dashed", color = "green", size=1) + # Add MLE for k1
    labs(x=TeX("Shape Parameter for Component 1 ($k_1$)"), y="Log-likelihood",
...
ggsave("test-flat-likelihood-prior.pdf", plot=plt.combined, device="pdf", width=10, height=5, units = "in")
