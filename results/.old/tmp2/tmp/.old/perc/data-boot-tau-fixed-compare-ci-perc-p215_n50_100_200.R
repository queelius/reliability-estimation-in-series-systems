...
# NOTE: removed two NA CI's from n=50 (was for `bca`, but removed
#       corresponding row for `perc` too for some reason
...
for (n in N) {
    for (p in P) {
        for (q in Q) {
            ...
            write.table(df, file = "data-boot-tau-fixed-perc-p215_n50_100_200.csv", sep = ",", row.names = FALSE,
                col.names = !file.exists("data-boot-tau-fixed-perc-p215_n50_100_200.csv"), append = TRUE)
        }
    }
}
