...
custom_ci <- function(theta, data, na.rm = TRUE, ...) {
    if (na.rm) {
    ...
    for (p in P) {
        for (q in Q) {
            ...
            df <- data.frame(
                ...
                shapes.lower.perc = shapes.lower.perc,
                shapes.upper.perc = shapes.upper.perc,
                scales.lower.perc = scales.lower.perc,
                scales.upper.perc = scales.upper.perc,                
                logliks = logliks)

            write.table(df, file = "data-boot-tau-fixed-bca-2.csv", sep = ",", row.names = FALSE,
                col.names = !file.exists("data-boot-tau-fixed-bca-2.csv"), append = TRUE)
