...
## System and Component Reliabilities {#sec:sys_weibull_reliability}
...
components <- rbind(components, "Series System" = c(NA, NA, mttf.sys, NA))
names(components) <- gsub("\\.", " ", names(components))
knitr::kable(components, caption = "Shape, Scale, Mean Time To Failure (MTTF), and Probability of Component Failure of Weibull Components in Series Configuration")
```
...
4. For each of these $R$ datasets, perform bootstrap resampling $B = 100$ times to create
...
### Scenario #1: Assessing the Impact of Sample Size

In this scenario, we use the well-designed series system described in Section \ref{sec:},
fix the masking probability to $p = 0.215$ (small masking), fix the right-censoring
quantile to $q = 0.825$, and vary the sample size $n$ from $?$ to $?$.
...
are proprotional to their scale parameters. However, when the shape parameters are not
