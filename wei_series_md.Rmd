...
For instance, suppose we observe the following sample in Table \ref{tab:identifiability}.
...
\begin{theorem}
If the series system has Weibull components with identical shape parameters, the component cause of
failure is conditionally independent of the system failure time:
$$
    \Pr\{K_i = j | \T_i = t_i \} = \Pr\{K_i = j\} = \frac{\lambda_j^{-k}}{\sum_{l=1}^{m} \lambda_l^{-k}}.
$$
\end{theorem}
\begin{proof}
By Theorem \ref{sec:test}, the conditional probability of the $j$\textsuperscript{th} component being the
cause of failure given the system failure time is given by
$$
\Pr\{K_i = j | \T_i = t\} = \frac{h_j(t;\v{\theta_j})}{\sum_{l=1}^m h_l(t;\v{\theta_l})}
    = \frac{\frac{k}{\lambda_j}\bigl(\frac{t}{\lambda_j}\bigr)^{k-1}}
        {\sum_{l=1}^m \frac{k}{\lambda_;}\bigl(\frac{t}{\lambda_l}\bigr)^{k-1}},
$$
which simplifies to the result given in the theorem.
\end{proof}

If the scale parameters of the components are also identical, then scale
parameter $\lambda^*$ is given by
$$
    \lambda^* = \frac{\lambda}{m^{1/k}}.
$$
In this case, the probability of the component cause of failure for any component
is given by $1/m$.

## System and Component Reliabilities

In addition to the survival function, a common measure of reliability is
mean time to failure (MTTF). The MTTF for the $j$\textsuperscript{th}
component is given by
\begin{equation}
\label{mttf-weibull}
\text{MTTF}_j = E(T_{i j}) = \lambda_j \Gamma\biggl(1 + \frac{1}{k_j}\biggr),
\end{equation}
...
probability of component failure as described in Theorem \ref{thm:}. For Weibull
components, this is given by
\begin{equation}
\label{eq:sys_weibull_component_failure_probability}
\Pr\{K_i = j\} = \int_0^\infty \frac{h_j(t;\v{\theta_j})}{\sum_{l=m}} dt
    =
\end{equation}
\begin{proof}
By Theorem \ref{thm:}, the probability of component failure is given by
$$
\Pr\{K_i = j\} = E_{\v\theta}\biggl\{\frac{h_j(\T_i;\v{\theta_j})}
    {\sum_{l=1}^m h_l(\T_i;\v{\theta_l})}\biggr\}.
$$
\end{proof}
...
3. For each of these $R$ datasets, compute the MLE.
...
7. Interpret the results and discuss the performance of the MLE estimator
