...
the process then repeated with a new synthetic sample.
...
\item[$k_j = 1$] The hazard function is constant with respect to time.
\item[$k_j > 1$] The hazard function increases with respect to time. For instance,
  this may occur as a result of components wearing out or aging over time.
\end{enumerate}
...
In addition to the reliability (survival) function, a common measure of
reliability is mean time to failure (MTTF). The MTTF for the $j$\textsuperscript{th}
...
described in Section \ref{sec:simstudy}.
