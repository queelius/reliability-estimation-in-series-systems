...
Roughly, we seek to determine the sample size required to detect a statistically significant difference in the
...
We generate samples from size $50$ to size $400$ from the well-designed series system described in
Section \ref{sec:sim_study}, Table \ref{tab:series-sys}, and we compute the LRT statistic for each
sample. We plot the results in Figure \ref{fig:lrt}.

```{r out.width='50%', fig.align="center", echo = F, fig.cap="Likelihood Ratio Test Statistic vs Sample Size"}
knitr::include_graphics("image/lrt.png")
```

The variability in $p$-values across different sample sizes may reflect the
complexity of the models being compared and the underlying data generating process.
The absence of a clear trend in the mean $p$-values suggests that the test's power
and significance levels may be influenced by other factors beyond sample size,
notably right-censoring and masking of component cause of failure.

Further investigation may include analyzing what happens when we vary
right-censoring and masking of component cause of failure, as well as considering
series systems which are not so well-designed, e.g., series systems with components
that have very different shapes and scales. It may also be interesting to consider
series systems with more components, e.g., $m = 20$.

The current analysis suggests that the reduced model is appropriate for the DGP
and sample sizes. With that said, we proceed to investigate the full model in
...
```{r samp-size-n-vs-stats-p215-scale, out.width='100%', fig.cap=c("Sample Size vs MLEs ($p = 0.215, q = 0.825$)","Sample Size ($n$) vs MLEs"), fig.align="center", echo = F}
...
- *Mean of MLEs for Shapes*: The red dashed line is the mean of shape MLEs. Unlike the scale MLEs,
  ...
  In Section \ref{sec:effect-censoring}, right-censoring was shown to have a very small
  effect of positively biasing the shape parameter estimates for right-censoring quantiles 
  arond $q = 0.82%, but not for the scale parameters. This may partly explain the positive
  bias in the shape parameter estimates for small sample sizes. However, in Section \ref{},
  we show that it may also be related to the masking probability $p = 0.215$. When the sample
  size is small, the data is not sufficient to overcome the masking effect, and so the MLE is
  positively biased due to the uncertainty about which component caused the system to fail.
...
- *Coverage Probability (CP)*: For the scale parameters, the 95% CI is well-calibrated
  for masking probabilities up to $p = 0.725$, which is really quite significant,
  ...
  The bootstrapped (BCa) confidence intervals seem fairly well-calibrated for most realistic
  masking probabilities, constructing CIs that are neither too wide nor too narrow,
  but when the masking is essentially a fair coin flip, one should probably take the CIs with
  a grain of salt.
...
- *Dispersion of MLEs*: The shaded regions representing the 95% quantile of
  ...
  component cause of failure. However, even for fairly significant masking, $p \leq 0.55$,
  the 95% quantiles are relatively narrow and the CP is well-calibrated, indicating that
  the MLEs are still relatively precise and accurate.
...
- *IQR of Bootstrapped CIs*: The IQR (vertical blue bars) show that the bootstrapped BCa
  ...
  Afte a masking probability of $p = 0.4$, the widht of the CIs rapidly increase, which is
  apparently necessary for the CPs to remain well-calibrated.
...
By Equation \eqref{eq:mttf-wei}, we see that MTTF$_j$ is proportional to the scale parameter $\lambda_j$, which means
when we decrease the scale parameter of a component, we proportionally decrease the MTTF.
In this scenario, we start with the well-designed series system described in Section \ref{sec:sim_study},
Table \ref{tab:series-sys}, and we will manipulate the MTTF of component 3 by changing its scale parameter,
$\lambda_3$, and observing the effect this has on the MLE. Since the other components had a similiar MTTF,
we will arbitrarily choose component 1 to represent the other components.

We vary the MTTF of component 3 from around $300$ to $1500$ and the other components have their MTTFs fixed
at around 900, as show in Table \ref{tab:series-sys}. We fix the masking probability to $p = 0.215$ (moderate masking),
...
We make two observations about Figure \ref{fig:mttf-vs-ci}:

- When the MTTF of component 3 is much smaller than the other components, the estimate of parameters of component 3 is precise (narrow CIs with high Probability
...
- When the MTTF of component 3 is much larger than the MTTF of the other components, then component 3 is much less likely to be the component cause of failure,
