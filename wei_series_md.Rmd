...
1. Set up simulation parameters:
    ...
    - Set a seed value for simulation study reproducibility. All of the code
      ...
      the seed value. The data we generate for our simulation study can be reproduced
      by anyone with the same seed value and R code.
   
    - Generate a range of sample sizes $n$ from $n = 30$ to $n = 200$.
     
    - Generate a range of Bernoulli masking probabilities from $p = 0$
      (no masking the component cause of failure) to $p = 0.4$ (significant masking)
    ...
    - Generate a range of right-censoring times $\tau$ based on quantiles for the
      Weibull series system, from the 50th quantile ($50\%$ of the observations
      are expected to be right-censored) to the 99th quantile (only $1\%$ of the
      observations are expected to be right-censored).

2. Generate $R = 1000$ data sets for each scenario (combination of $n$, $p$, and
   $\tau$).
