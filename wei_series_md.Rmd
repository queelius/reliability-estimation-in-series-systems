...
1. Set up simulation parameters:
    ...
    - The true parameter value $\v\theta$ for the Weibull series system
    (5 components) has shape parameters given by
    $$
        k_1 = `r theta["shape1"]`,
        k_2 = `r theta["shape2"]`,
        k_3 = `r theta["shape3"]`,
    ...
    - We generate MLEs for a range of sample sizes from $n = 10$ to $n = 200$.
     
    - We generate MLEs for a range of probabilities from $p = 0$ (no masking)
      to $p = 0.3$ (significant masking) for including each non-failed
      component in the candidate set using the Bernoulli candidate set model.

    - We use only two separate right-censoring times, $\tau$ set to $\infty$ (no
      right censoring) to and $\tau$ set to the $0.75$-quantile of the Weibull series
      system.

2. For each scenario (combination of $n$, $p$, and $\tau$), we generate $R = 200$
   data sets.

3. We estimate the parameters for each replication, giving us $R$ estimates of the
...
4. Using the empirical sampling distributions, we estimate the bias, variance, MSE,
   and coverage probability of the MLE for each scenario.

6. We analyze and visualize the results, e.g., by plotting the bias, variance, MSE,
   and coverage probability as a function of $n$ and $p$ for each $\tau$. 
   ...
   coverage probability change as a function of $n$ and $p$ for each $\tau$.
...
defined in Section \ref{}, to construct $95\%$-confidence intervals for $\v\theta$
to compute the coverage probability of the MLE for each scenario. A confidence
interval is said to be *well-calibrated* if the coverage probability is close to
the nominal level, $95\%$.
...
Since the asymptotic distribution of the MLE
may not be achieved for finite $n$, the bootstrap method is a useful tool for
estimating the sampling distribution of the MLE for finite $n$, in particular, for
