...
has a reliability and hazard functions given by the following theorem.
\begin{theorem}
...
Table \ref{tab:series-sys}, and we vary the right-censoring quantile ($q$) from $60\%$ to $100\%$
...
- *Coverage Probability (CP)*: For the scale parameters, the $95\%$ CI is well-calibrated
  ...
  but when the masking is severe and the sample size is relatively small, one should
...
- *Dispersion of MLEs*: The shaded regions representing the $95\%$ quantile of
  ...
  $p \leq 0.55$, the $95\%$ quantiles are relatively narrow and the CP is
  well-calibrated, indicating that the MLEs are still relatively precise and accurate.
...
Table \ref{tab:series-sys}, and we will manipulate the MTTF of component 3, MTTF$_3$, by changing its
...
In Figure \ref{mttf-vs-ci}, we show the effect of changing the scale parameter of component $3$, $lambda_3$,
but use MTTF$_3$ on the MLE for the shape and
scale parameters. The top four plots only show the effect
on the MLEs for the the shape and scale parameters of components $1$ and $3$. The MLEs for the remaining
components were similar to the MLE for the shape and scale parameters of component $1$. The bottom plot
shows the coverage probabilities for all parameters.
...
the most extreme scenarios, e.g., Bernoulli masking probabilities of $p = 0.7$ with a relatively small
...
were well-calibrated for the scale parameters

the bootstrapped CIs are quite wide to
account for this sensitivity when the sample contains insufficient information due to censoring.
  The 

- The BCa (bootstrapped) confidence intervals are well-calibrated in all but the most extreme circumstances,
  constructing CIs that are neither too wide nor too narrow, with two notable exceptions:

    - When the masking probability is essentially a fair coin flip, one should probably take the CIs with
        a grain of salt.

    - When the components have radically differentiated failure characteristics, the CIs for the parameters
        of the other components are quite wide, but the coverage probability is still well-calibrated.

# Simulation Study: Full Model vs Reduced Model {#sec:full-vs-reduced}
...
We take the well-designed series system described in Section \ref{sec:sim_study},
Table \ref{tab:series-sys}, and manipulate the shape parameter of the third
